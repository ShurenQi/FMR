...
load('retrainGoogLeNet.mat') % rotation augmentation
% load('retrainGoogLeNet2.mat') % rotation and noise augmentation
%% Load pretrained deep denoising model
...
%         [YPred,~] = classify(retrainGoogLeNet,imresize(NI,[NorSize NorSize])); % rotation augmentation
%         [YPred,~] = classify(retrainGoogLeNet2,imresize(NI,[NorSize NorSize])); % rotation and noise augmentation
            [noisyR,noisyG,noisyB] = imsplit(NI); denoisedRGB = cat(3,denoiseImage(noisyR,net),denoiseImage(noisyG,net),denoiseImage(noisyB,net)); [YPred,~] = classify(retrainGoogLeNet,imresize(denoisedRGB,[NorSize NorSize])); % rotation augmentation and denoising
...
%% Retrain GoogLeNet on rotation/noisy versions %%
