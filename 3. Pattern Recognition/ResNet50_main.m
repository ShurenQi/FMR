...
load('retrainResNet50.mat'); % rotation augmentation
% load('retrainResNet502.mat'); % rotation and noise augmentation
...
for VAR=0:0.05:0.3
    ...
    for i=1:1:TrnSize
        for j=0:1:35
            ...
            %             [YPred,~] = classify(retrainResNet50,imresize(NI,[NorSize NorSize])); % rotation augmentation
            %             [YPred,~] = classify(retrainResNet502,imresize(NI,[NorSize NorSize])); % rotation and noise augmentation
            [noisyR,noisyG,noisyB] = imsplit(NI); denoisedRGB = cat(3,denoiseImage(noisyR,net),denoiseImage(noisyG,net),denoiseImage(noisyB,net)); [YPred,~] = classify(retrainResNet50,imresize(denoisedRGB,[NorSize NorSize])); % rotation augmentation and denoising
...
%% Retrain ResNet-50 on rotation/noisy versions %%
